...
//!
//! Provides:
//! - DeltaState trait (object safe, with clone_box)
...
///
/// Important: `clone_box` provides an object-safe way to clone boxed deltas.
/// This allows `Box<dyn DeltaState + Send>` to implement `Clone`.
pub trait DeltaState: Send + Sync {
    ...
    /// Object-safe clone for boxed trait objects.
    fn clone_box(&self) -> Box<dyn DeltaState + Send>;
}
...
/// Implement `Clone` for boxed trait objects by delegating to `clone_box`.
impl Clone for Box<dyn DeltaState + Send> {
    fn clone(&self) -> Box<dyn DeltaState + Send> {
        self.clone_box()
    }
}
